...
pub fn base_coefficient_table_to_string(table: &[BaseAndCoefficient]) -> String {
    table.iter().map(|BaseAndCoefficient{base, coefficient}|
        std::format!("BaseAndCoefficient{{ base: I1F31::from_bits({:#010x}_u32 as i32), coefficient: I1F31::from_bits({:#010x}_u32 as i32) }}, ", base.to_bits() as u32, coefficient.to_bits() as u32)
    ).collect::<String>()
}
