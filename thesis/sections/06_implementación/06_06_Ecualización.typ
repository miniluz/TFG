...
frecuencia de su respectivo filtro. Estos componentes se escalan independientemente, aumentando o reduciendo el volumen
de cada banda antes de volverlas a añadir. #footnote[Usando la tabla `DB_LINEAR_AMLITUDE_TABLE.`] Se puede usar, por
ejemplo, para atenuar un rango de frecuencias en particular o aumentar las frecuencias agudas, subiendo o bajando el
volumen a sus componentes correspondientes. Idealmente, si no se escala ningún filtro, el componente no afecta la señal.
...
coeficientes en formato Q15. Las frecuencias correspondientes a cada filtro son:
...
filtros de primer orden por rendimiento, y bajo experimentación éstos parámetros dieron los mejores resultados (teniendo
en cuenta que almacenar los coeficientes de un filtro en un Q15 afecta considerablemente su respuesta).
