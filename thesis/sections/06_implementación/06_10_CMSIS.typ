...
`cmsis_dsp` (que únicamente puede ejecutarse en un chip ARM @ref_web_cmsis_dsp). Los tipos genéricos en Rust no tienen
un coste de rendimiento al ejecutar un programa. Esto es debido a que Rust _monomorfiza_ los tipos genéricos: dada una
función `f<T>(arg: T)`, con el tipo genérico `T`, si se llama con los tipos concretos `A` y `B` se generan dos
implementaciones de la función: `f(arg: A)` y `f(arg: B)` @ref_web_rust_generics. En este caso, si se define una función
`f<CmsisOperations>()`, ya que el ejecutable de Sparklet únicamente las llama con `CmsisNativeOperations`, únicamente se
genera la función `f()` equivalente que usa `CmsisNativeOperations`, sin coste en ejecución en comparación a llamar las
...
biblioteca que permite usar el mecanismo estándar de pruebas de Rust en un sistema empotrado con Embassy
@ref_web_rust_embedded_test. Estas pruebas están definidas en el módulo `cmsis_interface` con una macro, para garantizar
que las implementaciones son idénticas.
...
GitHub Actions. El único módulo que ha de ser probado en un sistema empotrado es `CmsisNative`.
