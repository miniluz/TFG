...
frecuencia. Hay diversos tipos de osciladores, pero el más usado en entornos empotrados por su eficiencia es el de tabla
de onda (_wavetable_). Como se ve en la /* TODO */, la onda se almacena en una tabla de longitud $L$.

// TODO: Ilustración de una wavetable.

Supongamos que la tabla tiene longitud $L$ y que la frecuencia de muestreo es $f_s = 48.000 "Hz"$. Si cada muestreo $s$
se envía el íncide de la tabla correspondiente a $s mod 1.000$, se envía una onda con la forma de la tabla a
$f = 48.000 "Hz" div 1.000 = 48 "Hz"$. Si en lugar se avanza dos índices cada muestra, usando $2 times s mod 1.000$ como
índice, se envía una onda con frecuencia $f = 48.000 "Hz" div 500 = 96 "Hz"$. En general, la relación entre la
frecuencia $f$ y el incremento del índice $i$ se da con la @eq_incremento.
...
Para que el error $e_"cents"$ sea imperceptible, tiene que ser menor a $6 "cents"$. Si queremos un error $f_e$
imperceptible para la frecuencia $f$ de la cuerda más grave de un bajo de 5 cuerdas, de unos $30 "Hz"$, el error tiene
que ser de $f_e = (2^(1/1.200))^(e_"cents") times f - f approx 0.10 "Hz"$. Dado que $f_s / 2^"bits" = f_e$, hacen falta
...
con la siguiente, $i + 1 mod 256$. Interpolar evita el ruido que genera redondear del índice. Mientras mayor es $r$, más
cerca se está de la siguiente muestra, por lo que se usa como el peso de la siguiente muestra. Por lo tanto, la salida
se calcula con la @eq_salida_interpolada.
...
#let fn = footnote[La idea original era hacerlo con evaluación `const`, que se ejecuta en compile-time
  ...
  `const`, ya que no dan los mismos resultados en todos los CPUs @ref_web_powf_not_const.]
