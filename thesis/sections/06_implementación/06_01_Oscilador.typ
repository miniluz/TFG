...
frecuencia. Hay diversos tipos de osciladores, pero una de particular eficiencia en entornos empotrados es la sínstesis
de tabla de onda (_wavetable_). Como se ve en la /* TODO */, la onda se almacena en una tabla de longitud $L$.
...
$48.000 "Hz" div 2^16 = 0,73 "Hz"$. Esta resolución es alta para las notas agudas, pero es insuficiente para las notas
...
Si queremos un error $f_e$ imperceptible para la frecuencia $f$ de la cuerda más grave de un bajo de 5 cuerdas, de unos
...
con la siguiente, $i + 1 mod 256$, evitando ruido por el redondeo de la acumulación del incremento al entero más
cercano. Mientras mayor es $r$, más cerca se está de la siguiente muestra, por lo que se usa como el peso de la
siguiente muestra. Por lo tanto, la salida se calcula con la @eq_salida_interpolada.
